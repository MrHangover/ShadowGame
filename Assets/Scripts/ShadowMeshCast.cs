...
//This class is essentially the same as the Test.cs, except it doesn't add colliders.
public class ShadowMeshCast : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        shadowObject = new GameObject("shadowFake");
    ...
    void Update()
    {
        if (localRenderer.isVisible)
        {    //Should only run if the object can be seen by the camera, doesn't seem to work though.
